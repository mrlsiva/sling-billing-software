                    ...
                    <div class="border border-light mt-3 p-2 rounded" 
                        ...
                        <div class="d-flex flex-wrap align-items-center gap-3">
                            <div>
                                ...
                                <p class="fs-10 my-1">${v.category} - ${v.sub_category}</p>
                                <p class="fs-10 my-1">${v.size_name} - ${v.colour_name}</p>
                            </div>
                            <div class="ms-lg-auto">
                                <div class="input-step border bg-body-secondary p-1 mt-1 rounded d-inline-flex overflow-visible">
                                    <button type="button" class="minus bg-light text-dark border-0 rounded fs-20 lh-1 h-100">-</button>
                                    <input type="number" class="qty-input text-dark text-center border-0 bg-body-secondary rounded h-100" value="1" min="0" max="${v.quantity}" readonly>
                                    <button type="button" class="plus bg-light text-dark border-0 rounded fs-20 lh-1 h-100" data-system_id="${productId}">+</button>
                                </div>
                            </div>
                        </div>
        ...
        select: function (event, ui) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#alt_phone").val(data.alt_phone).prop('disabled', !!data.alt_phone);
                    $("#name").val(data.name).prop('disabled', true);
                    $("#address").val(data.address).prop('disabled', true);
                    $("#pincode").val(data.pincode).prop('disabled', !!data.pincode);
                    ...
                    $("#dob").val(data.dob).prop('disabled', !!data.dob);
                    $("#gst").val(data.gst).prop('disabled', !!data.gst);
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        ...
                        $("#alt_phone").val(data.alt_phone).prop('disabled', !!data.alt_phone);
                        $("#name").val(data.name).prop('disabled', !!data.name);
                        $("#address").val(data.address).prop('disabled', !!data.address);
                        $("#pincode").val(data.pincode).prop('disabled', !!data.pincode);
                        ...
                        // Billing GST
                        $("#billing_gst").val(data.gst).prop('disabled', !!data.gst);
                    }
                },
                error: function () {
                    $("#customer").val('');
...
function appendPaymentRow(method, amount, extraData = {}) {
    ...
    if ((received + amt) > payable) {
        ...
        const event = new CustomEvent("toast", {
            detail: {
                text: "Received amount cant be greater than payable amount.",
                gravity: "top",
                position: "right",
                className: "success",
                duration: 5000,
