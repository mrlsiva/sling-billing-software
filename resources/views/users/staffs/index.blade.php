	...
	<div class="row">
		<div class="col-xl-12">
			<div class="card">
				...
				<form method="get" action="{{route('staff.index', ['company' => request()->route('company')])}}">
                    <div class="row mb-2 p-3">
                        <div class="col-md-11">
                            <div class="input-group">
                                ...
                                <span class="input-group-text" id="clearFilter" style="display: {{ request('name') ? 'inline-flex' : 'none' }}"><a href="{{route('staff.index', ['company' => request()->route('company')])}}" class="link-dark"><i class="ri-close-large-line align-middle fs-20"></i></a></span>
                            </div>
                        </div>
							...
							<tbody>
								@foreach($staffs as $staff)
									<tr>
										...
										<td>{{$staff->phone}}</td>
										<td>
										    <form action="{{ route('staff.status', ['company' => request()->route('company')]) }}" method="post">
										        @csrf
	...
	<div class="modal fade" id="staffAdd" tabindex="-1" aria-labelledby="staffAdd" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content" >
                ...
                <form class="row" action="{{route('staff.store', ['company' => request()->route('company')])}}" method="post" enctype="multipart/form-data">
                	@csrf
    ...
    <div class="modal fade" id="staffEdit" tabindex="-1" aria-labelledby="staffEdit" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content" >
                ...
                <form class="row" action="{{route('staff.update', ['company' => request()->route('company')])}}" method="post" enctype="multipart/form-data">
                	@csrf
	                <div class="modal-body">
