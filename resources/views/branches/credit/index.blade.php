		...
		<div class="col-xl-12">
			<div class="card">
				<div class="card-header d-flex justify-content-between align-items-center">
					...
					<div>
						<a href="{{route('branch.credit.history', ['company' => request()->route('company')])}}" class="btn btn-outline-primary btn-sm fw-semibold"><i class="ri-history-line"></i> History</a>
						@if(!request()->is('*credits/index'))
							<a href="{{route('branch.report.daily', ['company' => request()->route('company')])}}" class="btn btn-outline-primary btn-sm fw-semibold">Back</a>
						@endif
					</div>
				</div>
		        ...
		        @if(!request()->is('*credits/index'))
		        	<form method="get" action="{{ route('branch.credit', ['company' => request()->route('company'),'date' =>  request('date', now()->format('Y-m-d')) ]) }}">
		        @else
		        	<form method="get" action="{{ route('branch.credit.index', ['company' => request()->route('company') ]) }}">
		        @endif
                        ...
                        <div class="col-md-11">
                            <div class="input-group">
                                ...
                                <span class="input-group-text" id="clearFilter" style="display: {{ request('customer') ? 'inline-flex' : 'none' }}">
                                	@if(!request()->is('*credits/index'))
                                		<a href="{{ route('branch.credit', ['company' => request()->route('company'),'date' =>  request('date', now()->format('Y-m-d')) ]) }}" class="link-dark"><i class="ri-close-large-line align-middle fs-20"></i></a>
                                	@else
                                		<a href="{{ route('branch.credit.index', ['company' => request()->route('company') ]) }}" class="link-dark"><i class="ri-close-large-line align-middle fs-20"></i></a>
                                	@endif
                                </span>
                            </div>
                        </div>
