...
class orderController extends Controller
{
    ...
    public function online(Request $request,$company)
    {
        $orders = Order::where([['branch_id',Auth::user()->id],['shop_id',Auth::user()->parent_id],['is_online_order',1]])
        ->when(request('order'), function ($query) {
            $search = request('order');
            $query->where(function ($q) use ($search) {
                    // search by bill id
        ...
        return view('branches.orders.online',compact('orders'));
    }
    ...
    public function edit(Request $request,$company,$id)
    {
        ...
        return view('branches.orders.edit',compact('order','order_details','order_payment_details','payments','finances'));
    }
