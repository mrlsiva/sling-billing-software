...
class vendorLedgerController extends Controller
{
    ...
    public function index(Request $request, $vendor)
    {
        ...
        if (!$vendor) {
            return $this->errorResponse([], 404, 'Vendor not found.');
        }
        ...
        $query = PurchaseOrder::with('details')
        ...
        $refund = PurchaseOrderRefund::where([
            ['vendor_id', $vendor->id],
            ['need_to_deduct', 1],
        ])->sum('refund_amount');

        $totalGross  = PurchaseOrder::whereIn('id', $purchaseOrderIds)->sum('gross_cost');
        $vendor_payments = VendorPayment::where('vendor_id', $vendor->id)->get();
        $totalPaid   = $vendor_payments->sum('amount') - $refund;

        $remaining_opening_balance = VendorOpeningBalance::where([
            ['vendor_id', $vendor->id],
        ...
        return $this->successResponse([
            ...
            'balance'                   => $balance,
            'remaining_opening_balance' => $remaining_opening_balance,
        ], 200, 'Vendor ledger retrieved successfully.');
    }

    public function get_payments(Request $request, $vendor)
    {
        $payments = VendorPayment::with('payment')
            ->where('vendor_id', $vendor)
            ->orderBy('paid_on', 'desc')
            ->get();

        return $this->successResponse($payments, 200, 'Vendor payments retrieved successfully.');
    }
    ...
    public function store_payment(Request $request)
    {
        ...
        try {
            ...
            // Step 1: Clear vendor opening balance overdues first
            ...
            foreach ($overdues as $due) {
                ...
                $payAmount = min($amountToDistribute, $due->remaining_amount);
            ...
            // Step 2: Allocate to unpaid purchase orders
            ...
            // Step 3: Store remaining as prepaid balance
            ...
            DB::rollBack();
            return $this->errorResponse([], 500, 'Error processing payment: ' . $e->getMessage());
        }
    }
}